...
async def get_indicator_explanations(category: str | None = None, db: Session = Depends(get_db)):
    rows = list_indicator_explanations(db, category=category)
    return {"indicator_explanations": rows, "count": len(rows)}


@router.get("/indicator-explanations/{indicator_key}", response_model=IndicatorExplanationResponse)
async def get_indicator_explanation_detail(indicator_key: str, db: Session = Depends(get_db)):
