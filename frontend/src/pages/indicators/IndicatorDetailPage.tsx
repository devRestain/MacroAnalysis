    ...
    <div className="mx-auto max-w-7xl px-4 py-6 lg:px-6">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold">{snapshot?.label ?? indicatorKey}</h1>
          <p className="mt-1 font-mono text-xs text-text-muted">{indicatorKey}</p>
        </div>
        {snapshot && <Badge tone={snapshot.signal === 'red' ? 'red' : snapshot.signal === 'yellow' ? 'yellow' : 'green'}>{snapshot.signal}</Badge>}
      </div>

      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <Card title="Latest Observation" eyebrow="runtime signal">
          {snapshot ? (
        ...
        <Card title="Static Explanation" eyebrow="metadata">
        ...
        <Card
          ...
          action={<div className="flex gap-1">{periods.map((item) => <button key={item} onClick={() => setPeriod(item)} className={`px-2 py-1 text-xs ${period === item ? 'bg-accent/20 text-accent' : 'text-text-muted hover:bg-surface-hover'}`}>{item}</button>)}</div>}
        ...
        <Card title="Related Calendar Events" eyebrow="watch items">
          {relatedEvents.length > 0 ? relatedEvents.map((event) => (
            <a key={event.id} href="/calendar" className="block border-b border-surface-border py-3 last:border-0">
              <div className="text-sm font-medium">{event.displayName}</div>
              <div className="mt-1 text-xs text-text-muted">{fmtDate(event.eventDateLocal)} · {event.displayTime ?? event.eventTimeLocal ?? 'time pending'}</div>
            </a>
          )) : <EmptyState label="No related calendar events found." />}
        </Card>

        <Card title="Related News" eyebrow="recent context">
          {news.state.status === 'success' ? news.state.data.slice(0, 5).map((item) => (
            <a key={item.id} href={item.url ?? '/news'} className="block border-b border-surface-border py-3 last:border-0">
