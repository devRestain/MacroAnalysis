...
export function DashboardPage() {
  ...
  return (
    <PageFrame
      ...
      action={<button onClick={summary.reload} className="inline-flex items-center gap-2 rounded-full border border-surface-border bg-white px-4 py-2 text-xs font-bold text-text-secondary hover:border-accent/35 hover:text-accent"><RefreshCw size={13} />{t('dashboard.refresh')}</button>}
    >
      <div className="mb-6 grid gap-3 xl:grid-cols-[minmax(0,1.5fr)_repeat(3,minmax(0,0.5fr))]">
        <div className="flat-metric min-w-0">
          <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-text-muted">{t('dashboard.workspace')}</div>
          <h2 className="mt-2 text-2xl font-bold tracking-[-0.03em] text-text-primary">{t('dashboard.workspaceHeadline')}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-7 text-text-secondary">
            {t('dashboard.workspaceBody')}
          </p>
        </div>
        <MetricChip label={t('dashboard.metric.trackedSignals')} value={topSignals.length} />
        <MetricChip label={t('dashboard.metric.upcomingEvents')} value={upcoming.length} />
        <MetricChip label={t('dashboard.metric.newsItems')} value={data.newsPreview.length} />
      </div>

      <div className="mb-5 flex flex-wrap items-center gap-3">
        <DataFreshness updatedAt={data.updatedAt} />
        {data.fomc?.daysLeft != null && <Badge tone="blue">FOMC D-{data.fomc.daysLeft}</Badge>}
        {data.aiAsOfDate && <Badge>{t('dashboard.aiBriefing')} {data.aiAsOfDate}</Badge>}
      </div>

      <div className="grid gap-4 2xl:grid-cols-12">
        <Card title={t('dashboard.card.marketOverview')} eyebrow={t('dashboard.card.marketOverviewEyebrow')} tone="blue" className="2xl:col-span-7">
            ...
            {topSignals.map((item) => (
              <a key={item.key} href={`/indicators/${encodeURIComponent(item.key)}`} className="min-w-0 rounded-[18px] border border-surface-border bg-white p-4 hover:border-accent/30">
                <div className="flex min-w-0 items-start justify-between gap-2">
                  ...
                  <Badge tone={item.signal === 'red' ? 'red' : item.signal === 'yellow' ? 'yellow' : 'green'}>{label('signal', item.signal, item.signal)}</Badge>
                </div>
        ...
        <Card title={t('dashboard.card.aiHeadline')} eyebrow={t('dashboard.card.aiHeadlineEyebrow')} tone="amber" className="2xl:col-span-5">
          <div className="min-w-0">
            <p className="text-safe text-lg font-bold leading-8">{data.aiHeadline || t('dashboard.aiHeadlineEmpty')}</p>
            <a href="/ai" className="mt-5 inline-flex rounded-full border border-surface-border bg-white px-3 py-1.5 text-xs font-bold text-accent">{t('dashboard.openBriefings')}</a>
          </div>
        </Card>

        <Card title={t('dashboard.card.yieldCurve')} eyebrow={t('dashboard.card.yieldCurveEyebrow')} tone="green" className="2xl:col-span-4">
        ...
        <Card title={t('dashboard.card.equitySectors')} eyebrow={t('dashboard.card.equitySectorsEyebrow')} tone="default" className="2xl:col-span-4">
        ...
        <Card title={t('dashboard.card.calendarWatch')} eyebrow={t('dashboard.card.calendarWatchEyebrow')} tone="rose" className="2xl:col-span-4">
            ...
            <div className="divide-y divide-surface-border">
              {upcoming.map((event) => (
                <a key={event.id} href="/calendar" className="block min-w-0 py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="mt-1 truncate text-xs font-medium text-text-muted">{event.eventDateLocal} · {event.displayTime ?? event.eventTimeLocal ?? t('dashboard.timePending')}</div>
                </a>
              ))}
            </div>
        ...
        <Card title={t('dashboard.card.newsPreview')} eyebrow={t('dashboard.card.newsPreviewEyebrow')} tone="amber" className="2xl:col-span-6">
          <div className="divide-y divide-surface-border">
        ...
        <Card title={t('dashboard.card.sentimentPanel')} eyebrow={t('dashboard.card.sentimentPanelEyebrow')} tone="green" className="2xl:col-span-6">
              ...
              {sentimentRows.map((row) => (
                <div key={row.id} className="min-w-0 rounded-[16px] border border-surface-border bg-white p-3 text-sm">
                  <div className="flex min-w-0 justify-between gap-3">
                    <span className="truncate font-semibold">{row.actor ?? label('sourceType', row.sourceType, 'signal')} / {row.dimension ?? 'macro'}</span>
