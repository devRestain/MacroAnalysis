...
export function CommunicationsPage() {
  ...
  return (
    <div className="page-shell animate-fade-up">
      <div className="mb-6">
        <h1 className="text-3xl font-extrabold tracking-[-0.04em]">Communications</h1>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-text-secondary">Fed communications, FOMC details, and communication-scoped briefings kept separate from general calendar dates.</p>
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <Card title="FOMC Detail Context" eyebrow="meeting records" tone="rose">
              ...
              {fomc.state.data.meetings.slice(0, 8).map((meeting) => (
                <div key={`${meeting.id ?? meeting.date}`} className="rounded-[20px] border border-surface-border/80 bg-white/75 px-4 py-3">
                  <div className="flex justify-between gap-3">
                    <span className="text-sm font-bold">{meeting.displayName ?? 'FOMC Meeting'}</span>
        ...
        <Card title="Powell Communication Summary" eyebrow="typed AI summary" tone="blue">
          {powell.state.status === 'success' ? (
            <article>
              <h2 className="text-xl font-extrabold tracking-[-0.02em]">{powell.state.data.headline ?? 'Communication briefing'}</h2>
              <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-text-secondary">{powell.state.data.body ?? 'No summary body available.'}</p>
        ...
        <Card title="Communication Sentiment" eyebrow="optional pipeline" tone="amber" className="xl:col-span-2">
                ...
                {communicationSignals.slice(0, 8).map((signal) => (
                  <div key={signal.id} className="rounded-[20px] border border-surface-border/80 bg-white/78 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-sm font-bold">{signal.actor ?? 'communication'} / {signal.dimension ?? 'macro'}</span>
